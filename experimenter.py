...
#         self.elapsed = []
#
#         self.probability_SBX = []
#         self.probability_PCX = []
#         self.probability_DE = []
#         self.probability_UNDX = []
#         self.probability_SPX = []
#         self.probability_UM = []
#
#         print('initialize callback')
#
#     def __call__(self, algorithm):
#         if len(self.nfe) != 0:  # first run
#             if algorithm.nfe < self.nfe[-1]:  # reset callback
#                 self.nfe = []
#                 self.hypervolume = []
#                 self.decision_vars = []
#                 self.elapsed = []
#                 self.start_time = time.time()
#
#         self.probability_SBX.append(algorithm.variator.probabilities[0])
#         self.probability_PCX.append(algorithm.variator.probabilities[1])
#         self.probability_DE.append(algorithm.variator.probabilities[2])
#         self.probability_UNDX.append(algorithm.variator.probabilities[3])
#         self.probability_SPX.append(algorithm.variator.probabilities[4])
#         self.probability_UM.append(algorithm.variator.probabilities[5])
#
#         self.nfe.append(algorithm.nfe)
#         self.decision_vars.append(algorithm.fittest.variables)
#         self.elapsed.append(time.time() - self.start_time)
#
#         # Calculate hypervolume
#         self.hypervolume.append((algorithm.fittest.objectives[0]) / self.maximum)
#
#         logging.info(
#             f"NFE = {self.nfe[-1]}, Hypervolume is {self.hypervolume[-1]}"
#         )
...
def experiment(algorithms=[],
    ...
    generator = evaluate_job_generator(algorithms, problems, seeds, nfe, callback, display_stats)
    ...
    for job in job_results:
        ...
        results[job.algorithm_name][job.problem_name].append(job.instance)
        count += 1
