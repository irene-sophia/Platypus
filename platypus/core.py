...
class Algorithm(object):
    ...
    def run(self, condition, callback=None):
        ...
        self.result = {}
        self.result['nfe'] = callback.nfe
        self.result['best_score'] = callback.hypervolume
        self.result['elapsed time'] = callback.elapsed

        self.result['probability_SBX'] = callback.probability_SBX
        self.result['probability_PCX'] = callback.probability_PCX
        self.result['probability_DE'] = callback.probability_DE
        ...
        for u in range(self.problem.U):
            col_name = 'pi_' + str(u)
            var_u = []
            for i in range(len(callback.decision_vars)):
                var_u.append(np.floor(callback.decision_vars[i][u]))
            self.result[col_name] = var_u
